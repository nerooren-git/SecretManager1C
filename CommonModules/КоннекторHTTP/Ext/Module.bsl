...
Функция PatchJson(URL, Json, ДополнительныеПараметры = Неопределено, Сессия = Неопределено) Экспорт
	...
	ПараметрыПреобразованияJSON =
		ВыбратьЗначение(Неопределено, ДополнительныеПараметры, "ПараметрыПреобразованияJSON", Неопределено);
	Возврат КакJson(ВызватьHTTPМетод(ТекущаяСессия, "PATCH", URL, ДополнительныеПараметры), ПараметрыПреобразованияJSON);
...
Функция ОтправитьЗапрос(Сессия, ПодготовленныйЗапрос, Настройки)
	...
	ПодготовленныйОтвет.Вставить("ТелоКакСтрока", Ответ.ПолучитьТелоКакСтроку());
...
Функция НастройкиПодключения(Метод, URL, ДополнительныеПараметры)
	...
	НастройкиПереопределяемый = Новый Структура;
	НастройкиПереопределяемый.Вставить("Таймаут", Таймаут(ДополнительныеПараметры));
	НастройкиПереопределяемый.Вставить("РазрешитьПеренаправление", РазрешитьПеренаправление);
	НастройкиПереопределяемый.Вставить("ПроверятьSSL", ПроверятьSSL);
	НастройкиПереопределяемый.Вставить("КлиентскийСертификатSSL", КлиентскийСертификатSSL);
	НастройкиПереопределяемый.Вставить("Прокси", Прокси);
	НастройкиПереопределяемый.Вставить("МаксимальноеКоличествоПовторов", МаксимальноеКоличествоПовторов);
	НастройкиПереопределяемый.Вставить("ПовторятьДляКодовСостояний", ПовторятьДляКодовСостояний);
	НастройкиПереопределяемый.Вставить("КоэффициентЭкспоненциальнойЗадержки", КоэффициентЭкспоненциальнойЗадержки);
	НастройкиПереопределяемый.Вставить("МаксимальноеВремяПовторов", МаксимальноеВремяПовторов);

	Возврат НастройкиПереопределяемый;
